...
interface ChatInputProps {
	nowChattingId: number;
	onVideo: boolean;
	nowChattingName: React.ReactNode;
}
...
export default function ChatInput(props: ChatInputProps) {
	...
	useEffect(() => {
		// 光标位置
		const inputDOM = inputRef.current as HTMLDivElement;
		const caretOffset = getCaretPosition(inputDOM);
		let htmlStr = rawHtml;
	...
	// 上一轮光标位置
	const [lastEditRange, setLastEditRange] = useState<Range | undefined>(undefined);
	...
	return (
		<>
											...
											for (
												...
												if (i === selection.anchorOffset) {
													inputDOM.insertBefore(
														emojiText,
														inputDOM.childNodes[i]
													);
												}
											}
										} else {
											// 否则直接插入一个表情元素
											inputDOM.appendChild(emojiText);
										}
										// 创建新的光标对象
			...
			<div
				...
				spellCheck={false}
				onInput={(evt) => {
					setRawHtml((evt.target as HTMLDivElement).innerHTML);
				}}
				onKeyUp={() => {
					const selection = getSelection() as Selection;
					setLastEditRange(selection.getRangeAt(0));
...
function setCaretPosition(element: HTMLDivElement, pos: number) {
...
function createRange(node: HTMLElement, chars: { count: any; }, range?: Range): Range {
	...
	if (chars.count === 0) {
		...
		if (node.nodeType === Node.TEXT_NODE) {
			if ((node.textContent as string).length < chars.count) {
				chars.count -= (node.textContent as string).length;
			} else {
				range.setEnd(node, chars.count);
				chars.count = 0;
			}
			...
			for (const childNode of node.childNodes) {
				range = createRange(childNode as HTMLElement, chars, range);
				if (chars.count === 0) {
					break;
				}
			}
