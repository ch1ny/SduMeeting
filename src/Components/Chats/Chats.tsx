...
export default function Chats() {
	...
	const [friendsList, dispatchFriendsList] = useReducer
		<(state: Map<number, any>, action: { type: string, payload?: any, friend?: any, friends?: any }) => Map<number, any>>((state, action) => {
			switch (action.type) {
				case 'addFriend':
	...
	return (
		<ChatRTCContext.Provider value={chatRtc}>
			<div id='chatsHeader'>
				<div id='chatsTitle'>{nowChatting ? (nowChatting as any).username : '聊天界面'}</div>
				<div id='controlPanel'>
					<div
						...
						for (const [friendId, friend] of friendsList) {
							friends.push(
								<FriendBubble
									...
									unreadNumber={unreadNumber[`${friend.uid}` as keyof typeof unreadNumber]}
									style={{
											...
											<List.Item.Meta
												avatar={
													<Avatar
														src={
															(item as any).profile
																? `http://meeting.aiolia.top:8080/file/pic/user/${(item as any).uid}.${(item as any).profile}`
																: (item as any).profile
														}
														size={50}
												...
												onClick={({ nativeEvent }) => {
													...
													for (const ele of path) {
														...
														switch ((ele as Node).nodeName) {
															...
															case 'BUTTON':
																// INFO: 1 是拒绝，2 是同意
																result = (ele as HTMLElement).classList.contains(
																	'ant-btn-dangerous'
																)
																	? REJECT_FRIEND_REQUEST
																	: ACCEPT_FRIEND_REQUEST;
